...
public class ActionStatsOperationsManager : ITestManager<ActionStatsOperationsManager, ActionStats?>
{
    ...
    private void ValidateBeOfTypeOperation(Reason? reason, Type? type)
    {
        ExecutionEngine<ActionStatsOperationsManager, ActionStats?>
            .New(this)
            .WithOperation(ReferenceBeOfTypeValidator<ActionStats?>.New(PrincipalChain, type!))
            .WithTemplate(
                (template, operation) =>
                    template
                        .WithSubject(PrincipalChain.GetSubject())
    ...
    private void ValidateNotBeOfTypeOperation(Reason? reason, Type? type)
    {
        ExecutionEngine<ActionStatsOperationsManager, ActionStats?>
            .New(this)
            .WithOperation(ReferenceNotBeOfTypeValidator<ActionStats?>.New(PrincipalChain, type!))
            .WithTemplate(
                (template, operation) =>
                    template
                        .WithSubject(PrincipalChain.GetSubject())
