...
internal class TestFrameworkHandler
{
    ...
    public static List<EnumKeyValueData<int, string>> TestFrameworkAssemblyNames { get; } =
        Frameworks
            .GetKeyValues<int, string>("assembly")
            .Where(x => !x.ObjectValue.IsNull())
            .ToList();

    public static List<EnumKeyValueData<int, string>> TestFrameworkExceptionNamespace { get; } =
        Frameworks
            .GetKeyValues<int, string>("exception")
            .Where(x => !x.ObjectValue.IsNull())
            .ToList();

    private List<EnumBooleanData<int>> TestFrameworkForceAssembly { get; } =
        Frameworks
            .GetBooleanValues<int>()
            .Where(
                x =>
                    Frameworks
                        .GetKeyValues<int, string>("assembly")
                        .Any(a => a.Value == x.Value && !a.ObjectValue.IsNull())
            )
            .ToList();
    ...
    public Assembly? GetFrameworkForExplicit(TestFramework framework)
    {
        var assemblyEntry = TestFrameworkAssemblyNames.FirstOrDefault(
            x => x.Value == (int)framework
        );
        ...
        try
        {
            return Assembly.Load(new AssemblyName(assemblyEntry.ObjectValue));
        }
        catch
        {
            return null;
