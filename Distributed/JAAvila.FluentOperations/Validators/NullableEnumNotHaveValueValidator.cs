...
/// </summary>
internal class NullableEnumNotHaveValueValidator<T>(PrincipalChain<T?> chain)
    : IValidator,
        IRuleDescriptor
    where T : struct, Enum
{
    public static NullableEnumNotHaveValueValidator<T> New(PrincipalChain<T?> chain) => new(chain);
