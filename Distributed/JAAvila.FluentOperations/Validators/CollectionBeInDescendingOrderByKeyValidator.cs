using JAAvila.FluentOperations.Contract;

namespace JAAvila.FluentOperations.Validators;

/// <summary>
/// Validates that the elements in the collection are in descending order
/// when projected through the specified key selector.
/// </summary>
internal class CollectionBeInDescendingOrderByKeyValidator<T, TKey>(
    PrincipalChain<IEnumerable<T>> chain,
    Func<T, TKey> keySelector
) : IValidator, IRuleDescriptor
    where TKey : IComparable<TKey>
{
    public static CollectionBeInDescendingOrderByKeyValidator<T, TKey> New(
        PrincipalChain<IEnumerable<T>> chain,
        Func<T, TKey> keySelector
    ) => new(chain, keySelector);

    public string Expected { get; }
    public string ResultValidation { get; set; }
    public string MessageKey => "Collection.BeInDescendingOrderByKey";
    string IRuleDescriptor.OperationName => "BeInDescendingOrderByKey";
    Type IRuleDescriptor.SubjectType => typeof(IEnumerable<>);
    IReadOnlyDictionary<string, object> IRuleDescriptor.Parameters =>
        new Dictionary<string, object>();

    public bool Validate()
    {
        var list = chain.GetValue().ToList();

        if (list.Count <= 1)
        {
            return true;
        }

        var comparer = Comparer<TKey>.Default;

        for (var i = 0; i < list.Count - 1; i++)
        {
            var currentKey = keySelector(list[i]);
            var nextKey = keySelector(list[i + 1]);

            if (comparer.Compare(currentKey, nextKey) >= 0)
            {
                continue;
            }

            ResultValidation =
                $"The resulting collection was expected to be in descending order by the selected key, "
                + $"but element at index {i} (key: '{currentKey}') was less than element at index {i + 1} (key: '{nextKey}').";
            return false;
        }

        return true;
    }

    public Task<bool> ValidateAsync()
    {
        return Task.FromResult(Validate());
    }
}
